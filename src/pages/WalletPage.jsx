      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4">
        ...
        <div className="bg-gradient-to-br from-red-800 to-red-900 rounded-xl p-6 mb-4 shadow-2xl border border-red-700/50">
          <div className="flex flex-col items-center">
            ...
            {/* Redeem Button */}
            <Link to ="/reedem" className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-10 py-2 rounded-full font-semibold transition-all duration-300 shadow-lg hover:shadow-amber-500/25">
              Redeem
            </Link>
          </div>
        </div>

        {/* See Redeem History Link */}
        <div className="text-center mb-4">
          <Link to="/reedem-history" className="text-amber-400 text-sm hover:text-amber-300 transition-colors">
            See Redeem History
          </Link>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            {transactions.map((tx) => (
              <div 
                key={tx.id} 
                className="flex items-center justify-between py-3 border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors px-2 rounded"
              >
                <div>
                  <h3 className="text-white text-sm font-medium">{tx.title}</h3>
                  <p className="text-gray-400 text-xs">{tx.desc}</p>
                  <p className="text-gray-500 text-xs">{tx.time}, {tx.date}</p>
                </div>
