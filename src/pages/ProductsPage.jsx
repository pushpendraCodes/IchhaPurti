    ...
    <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 relative group">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => Navigate(`/product/${product.id || product._id}`)} className="flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded-lg font-medium text-xs text-white transition-all hover:opacity-90" style={{ background: 'linear-gradient(135deg, #C9A227 0%, #a07d1c 100%)' }}>
            <Eye size={14}/> Details
          </button>
        </div>
      </div>
...
export default function ProductsPage() {
  ...
  const fetchProducts = async (pageNumber) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/user/products/getAllProducts?page=${pageNumber}&limit=${limit}`,
        {
          method: "GET",
          headers: {
  ...
  if (products.length === 0) return /* your no products UI */;
